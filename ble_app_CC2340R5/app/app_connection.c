...
void Connection_ConnEventHandler(uint32 event, BLEAppUtil_msgHdr_t *pMsgData)
{
    switch(event)
    {
        case BLEAPPUTIL_LINK_ESTABLISHED_EVENT:
        {
            ...
            Display_printf(handle, 0, 0, "Conn status: Established - Connected to %s connectionHandle = %d", BLEAppUtil_convertBdAddr2Str(gapEstMsg->devAddr), gapEstMsg->connectionHandle);

            /*! Print the number of current connections */
            Display_printf(handle, 0, 0, "Connections number: %d", linkDB_NumActive());
            ProximityProfile_on_connect();
        ...
        case BLEAPPUTIL_LINK_TERMINATED_EVENT:
        {
            ...
            /*! Print the peer address and connection handle number */
            Display_printf(handle, 0, 0, "Conn status: Terminated - connectionHandle = %d reason = %d", gapTermMsg->connectionHandle, gapTermMsg->reason);

            /*! Print the number of current connections */
            Display_printf(handle, 0, 0, "Connections number: %d", linkDB_NumActive());
            ProximityProfile_on_disconnect();
        ...
        case BLEAPPUTIL_LINK_PARAM_UPDATE_EVENT:
        {
            ...
            if (linkDB_GetInfo(pPkt->connectionHandle, &linkInfo) ==  SUCCESS)
            {
              ...
              {
                  Display_printf(handle, 0, 0, "Conn status: Params update - connectionHandle = %d", pPkt->connectionHandle);
              }
              else
              {
                  Display_printf(handle, 0, 0, "Conn status: Params update failed - 0x%x connectionHandle = %d", pPkt->opcode, pPkt->connectionHandle);
              }
            }
    ...
    switch (event)
    {
        case BLEAPPUTIL_HCI_COMMAND_STATUS_EVENT_CODE:
        {
            ...
            switch ( event )
            {
              case HCI_LE_SET_PHY:
              {
                  ...
                  {
                      Display_printf(handle, 0, 0, "Conn status: Phy update - failure, peer does not support this");
                  }
                  else
                  {
                      Display_printf(handle, 0, 0, "Conn status: Phy update - 0x%02x", pHciMsg->cmdStatus);
                  }
                  break;
              }
        ...
        case BLEAPPUTIL_HCI_LE_EVENT_CODE:
        {
            ...
            if (pPUC->BLEEventCode == HCI_BLE_PHY_UPDATE_COMPLETE_EVENT)
            {
              ...
              {
                  Display_printf(handle, 0, 0, "Conn status: Phy update failure - connHandle = %d", pPUC->connHandle);
              }
              else
              {
                  ...
                  Display_printf(handle, 0, 0, "Conn status: Phy update - connHandle = %d PHY = %s", pPUC->connHandle, currPhy);

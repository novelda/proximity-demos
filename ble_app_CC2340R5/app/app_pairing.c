...
void Pairing_pairStateHandler(uint32 event, BLEAppUtil_msgHdr_t *pMsgData)
{
    switch(event)
    {
        ...
        {
            Display_printf(handle, 0, 0, "Pairing Status: Started - connectionHandle = %d status = %d", ((BLEAppUtil_PairStateData_t *)pMsgData)->connHandle, ((BLEAppUtil_PairStateData_t *)pMsgData)->status);

            break;
        }
        case BLEAPPUTIL_PAIRING_STATE_COMPLETE:
        {
            Display_printf(handle, 0, 0, "Pairing Status: Complete - connectionHandle = %d status = %d", ((BLEAppUtil_PairStateData_t *)pMsgData)->connHandle, ((BLEAppUtil_PairStateData_t *)pMsgData)->status);
        ...
        {
            Display_printf(handle, 0, 0, "Pairing Status: Encrypted - connectionHandle = %d status = %d", ((BLEAppUtil_PairStateData_t *)pMsgData)->connHandle, ((BLEAppUtil_PairStateData_t *)pMsgData)->status);
            break;
        }

        case BLEAPPUTIL_PAIRING_STATE_BOND_SAVED:
        {
            Display_printf(handle, 0, 0, "Pairing Status: Bond saved - connectionHandle = %d status = %d", ((BLEAppUtil_PairStateData_t *)pMsgData)->connHandle, ((BLEAppUtil_PairStateData_t *)pMsgData)->status);
            break;
        }

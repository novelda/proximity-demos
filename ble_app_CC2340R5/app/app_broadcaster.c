...
extern static Display_Handle handle;
...
void Broadcaster_AdvEventHandler(uint32 event, BLEAppUtil_msgHdr_t *pMsgData)
{
    ...
    switch(event)
    {
        ...
        {
            Display_printf(handle, 0, 0, "Adv status: Started - handle: %d", advData->pBuf->advHandle);
            break;
        }

        case BLEAPPUTIL_ADV_END_AFTER_DISABLE:
        {
            Display_printf(handle, 0, 0, "Adv status: Ended - handle: %d", advData->pBuf->advHandle);
            break;
        }

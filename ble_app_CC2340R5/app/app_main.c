...
#define DEV_NAME "Proximity";

uint8_t attNoveldaDeviceName[]= DEV_NAME;
...
BLEAppUtil_GeneralParams_t appMainParams =
{
    ...
    .deviceNameAtt = attNoveldaDeviceName,
};
...
void App_StackInitDoneHandler(gapDeviceInitDoneEvent_t *deviceInitDoneData)
{
    ...
    Display_printf(handle, 0, 0, "********** Novelda Proximity example **********");

    // Print the device ID address
    Display_printf(handle, 0, 0, "BLE ID Address: %s", BLEAppUtil_convertBdAddr2Str(deviceInitDoneData->devAddr));

    if ( appMainParams.addressMode > ADDRMODE_RANDOM)
    {
      // Print the RP address
        Display_printf(handle, 0, 0, "BLE RP Address: %s", BLEAppUtil_convertBdAddr2Str(GAP_GetDevAddress(FALSE)));
    }
